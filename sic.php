...
if(!empty($_POST['comment']) && !empty($_GET['id'])){
    ...
    $data_sic = sqlsrv_query($conn, $sql, array(), ['Scrollable' => SQLSRV_CURSOR_KEYSET]);
    $count = sqlsrv_num_rows($data_sic);
    ...
    if($count){
        ...
        $raw_query = "INSERT INTO el_sic_comment (id_sic, user_npk, comment, created_at)
                        VALUES ('$id_sic', '$user_npk', '$comment', '$date')";
        $query = sqlsrv_query($conn, $raw_query);
    }

    header("Location:sic.php?id=".$_GET['id']);
}

if(!empty($_GET['action']) && $_GET['action']=='deletecomment' && !empty($_GET['id']) && !empty($_GET['commentid'])){
    $sql = "SELECT * FROM el_sic_comment WHERE id=".$_GET['commentid'];
    $data_sic = sqlsrv_query($conn, $sql, array(), ['Scrollable' => SQLSRV_CURSOR_KEYSET]);
    $count = sqlsrv_num_rows($data_sic);

    if($count){
        $raw_query = "DELETE FROM el_sic_comment WHERE id=".$_GET['commentid'];
        $query = sqlsrv_query($conn, $raw_query);
    }
    ...
    <div class="container-fluid">
        <div class="card">
            ...
            <div class="card-header ">
                <h4 class="card-title">
                    Sharing is Caring Eksternal
                    <?php if(!empty($_GET['id'])): ?>
                        <button class="btn btn-primary float-right" onclick="window.location.replace('sic.php')">Back</button>
                    <?php else: ?>
                            ...
                            <div class="row" style="margin:20px 0 20px 0;">
                                    ...
                                    <div class="container">
                                        ...
                                        <div class="row">
                                            <div class="col-12" style="margin-bottom:30px;">
                                                ...
                                                <?php
                                                    $sql = "SELECT A.*, U.user_nama FROM el_sic_comment A
                                                ...
                                                <div style="width:100%; display:flex;">
                                                    <div style="width:50%;">
                                                        <?php if($pagination['page']>1): ?>
                                                            <a href="sic.php?id=<?=$_GET['id']?>&page=<?=$pagination['page']-1?>">Newer Comment</a>
                                                        <?php endif; ?>
                                                    </div>
                                                    <div style="width:50%; text-align:right;">
                                                        <?php if($pagination['page']<$pagination['total_page']): ?>
                                                            <a href="sic.php?id=<?=$_GET['id']?>&page=<?=$pagination['page']+1?>">Older Comment</a>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                ...
                                <?php if(empty($count)): ?>
                                    <?php
                                        ...
                                        $sql = "SELECT * FROM el_sic $search ORDER BY id DESC";
                                        ...
                                        $sql .= " OFFSET ".$pagination['start']." ROWS FETCH NEXT ".$pagination['limit']." ROWS ONLY";
                                        $data_sic = sqlsrv_query($conn, $sql, array(),['Scrollable' => SQLSRV_CURSOR_KEYSET]);
                                        // $data_sic = sqlsrv_num_rows($data_sic);
                                        // $data_sic = sqlsrv_fetch_array($data_sic, SQLSRV_FETCH_ASSOC);
                                    
                                        // print_debug($sql, false);
                                        // print_debug($data_sic);
                                        while($data = sqlsrv_fetch_array($data_sic, SQLSRV_FETCH_ASSOC)):
                                    ?>
                                        <div class="col-sm-3">
