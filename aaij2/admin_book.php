<?php
    ...
    if(!empty($_GET['id']) && !empty($_GET['do']) && $_GET['do'] = "delete"){
        $sql = "SELECT el_book.*, el_book_genre.genre FROM el_book
                LEFT JOIN el_book_genre ON el_book.id_genre = el_book_genre.id
                WHERE el_book.id=".$_GET['id'];
        $data_book = sqlsrv_query($conn, $sql);
        $data = sqlsrv_fetch_array($data_book, SQLSRV_FETCH_ASSOC);
        ...
        if(!empty($data)){
            ...
            $sql = "DELETE FROM el_book WHERE id=".$_GET['id'];
    ...
    $data_book = sqlsrv_query($conn, $sql, array(),['Scrollable' => SQLSRV_CURSOR_KEYSET]);
    // $data_book = sqlsrv_num_rows($data_book);

    // print_debug($sql, false);
    // print_debug($data_book);
?>
                            ...
                            <div class="card strpied-tabled-with-hover" style="padding: 10px 20px;">
                                <div class="card-header ">
                                    <div class="float-left">
                                        <h4 class="card-title">Book Master</h4>
                                        <p class="card-category">Here is list of book course</p>
                                    </div>
                                    <div class="float-right">
                                        <a href="admin_book_create.php" class="btn btn-primary">+ Tambah</a>
                                    </div>
                                </div>
                                <div class="card-body table-full-width table-responsive">
                                    <table class="table table-hover table-striped">
                                        ...
                                        <tbody>
                                            <?php $i=$pagination['start']+1; while($data = sqlsrv_fetch_array($data_book, SQLSRV_FETCH_ASSOC)): ?>
                                                <tr style="cursor:pointer;">
                                                    <td class="text-center"><?=$i++?></td>
                                                    <td class="text-center"><?=$data['date']->format('d M Y')?></td>
                                                    <td class="text-center"><?=$data['genre']?:'-'?></td>
                                                    <td><?=$data['title']?></td>
                                                    <td width="50%"><?=$data['sinopsis']?></td>
                                                    
                                                    <td class="text-right">
                                                        <a class="btn btn-info" style="color:white; width:100px;" href="admin_book_edit.php?id=<?=$data['id']?>">
                                                            <i class="fa fa-pencil"></i> Edit
                                                        </a>
                                                        <a class="btn btn-danger" style="color:white; width:100px;" href="admin_book.php?do=delete&id=<?=$data['id']?>">
                                        ...
                                        <ul class="pagination float-right">
                                            <li class="paginate_button page-item <?=$pagination['page']==1?'disabled':''?>">
                                                <a href="admin_book.php?page=<?=$pagination['page']!=1?$pagination['page']-1:''?>" class="page-link">Previous</a>
                                            </li>
                                            <?php for($i=$pagination['first'];$i<=$pagination['last'];$i++): ?>
                                                <li class="paginate_button page-item <?=$pagination['page']==$i?'active':''?>">
                                                    <a href="admin_book.php?page=<?=$i?>" class="page-link"><?=$i?></a>
                                                </li>
                                            <?php endfor; ?>
                                            <li class="paginate_button page-item <?=$pagination['page']>=$pagination['total_page']?'disabled':''?>">
                                                <a href="admin_book.php?page=<?=$pagination['page']<$pagination['total_page']?$pagination['page']+1:''?>" class="page-link">Next</a>
                                            </li>
                                        </ul>
                                </div>
                            </div>
