<?php
    ...
    if(!empty($_GET['id']) && !empty($_GET['do']) && $_GET['do'] = "delete"){
        ...
        $sql = "SELECT el_module.*FROM el_module
                WHERE el_module.golongan <> '' AND el_module.id=".$_GET['id'];
        ...
        if(!empty($data)){
            if(!empty($data['detail'])){
                foreach($data['detail'] as $detail){
                    ...
                    $sql = "DELETE FROM el_module_detail WHERE id=".$detail['id'];
            ...
            $sql = "DELETE FROM el_module WHERE id=".$_GET['id'];
        ...
        echo "<script>
                alert('".$message."');
                window.location.href='admin_module_wajib.php';
            </script>";
    }

    // Setup Pagination
    $sql = "SELECT el_module.* FROM el_module
            WHERE el_module.golongan <> ''
            ORDER BY el_module.id DESC";
...
<body>
    <div class="wrapper">
        ...
        <?php
        $page = 'admin_module_wajib'; // current page is about, do the same for other page
        $collapsePage = 'menu3';
        include('sidebar.php');
        ?>
        <!-- Include End Sidebar -->
                            ...
                            <div class="card strpied-tabled-with-hover" style="padding: 10px 20px;">
                                <div class="card-header ">
                                    <div class="float-left">
                                        <h4 class="card-title">Module Wajib</h4>
                                        <p class="card-category">List of Module Data</p>
                                    </div>
                                    <div class="float-right">
                                        <a href="admin_module_wajib_create.php" class="btn btn-primary">+ Tambah</a>
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-hover table-striped">
                                        ...
                                        <tbody>
                                            <?php $i=$pagination['start']+1; while($data = sqlsrv_fetch_array($data_module, SQLSRV_FETCH_ASSOC)): ?>
                                                <tr style="cursor:pointer;">
                                                    ...
                                                    <td width="20px" class="text-center"><?=$data['golongan']?></td>
                                                    <td><?=$data['title']?></td>
                                                    <td class="text-center"><?=$data['closed']->format('d M Y')?></td>
                                                    <td class="text-center"><?=$data['created_at']->format('d M Y')?></td>
                                                    <td class="text-center">
                                                        <a class="btn btn-link" href="module.php?id=<?=$data['id']?>" target="_blank">
                                                            <i class="fa fa-eye"> Lihat File</i>
                                                        </a>
                                                    </td>
                                                    <td class="text-right">
                                                        <!-- <a class="btn btn-info" style="color:white; width:100px;" href="javascript:void(0);">
                                                            <i class="fa fa-pencil"></i> Edit
                                                        </a> -->
                                                        <a class="btn btn-danger" style="color:white; width:100px;" href="admin_module_wajib.php?do=delete&id=<?=$data['id']?>">
                                        ...
                                        <ul class="pagination float-right">
                                            <li class="paginate_button page-item <?=$pagination['page']==1?'disabled':''?>">
                                                <a href="admin_module_wajib.php?page=<?=$pagination['page']!=1?$pagination['page']-1:''?>" class="page-link">Previous</a>
                                            </li>
                                            <?php for($i=$pagination['first'];$i<=$pagination['last'];$i++): ?>
                                                <li class="paginate_button page-item <?=$pagination['page']==$i?'active':''?>">
                                                    <a href="admin_module_wajib.php?page=<?=$i?>" class="page-link"><?=$i?></a>
                                                </li>
                                            <?php endfor; ?>
                                            <li class="paginate_button page-item <?=$pagination['page']>=$pagination['total_page']?'disabled':''?>">
                                                <a href="admin_module_wajib.php?page=<?=$pagination['page']<$pagination['total_page']?$pagination['page']+1:''?>" class="page-link">Next</a>
                                            </li>
                                        </ul> 
    ...
    function addRow(){
        ...
        var html = '<div id="row'+id+'">\
                        ...
                        <div class="form-left-to-w3l">\
                            ...
                            <textarea name="isi[]" id="mytextarea" class="form-control" required></textarea>\
                            <div class="clear"></div>\
                        </div><br>\
                        <div class="form-left-to-w3l">\
