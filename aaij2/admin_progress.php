<?php
    ...
    // Setup Pagination
    $whereclause = "el_module.golongan <> ''";
    if(!empty($_GET['pilihan'])){
        $whereclause = "COALESCE(el_module.golongan,0)=0";
    }
...
<body>
    <div class="wrapper">
        ...
        <?php
        $page = 'admin_progress'; // current page is about, do the same for other page
        $collapsePage = 'menu3';
        include('sidebar.php');
        ?>
        <!-- Include End Sidebar -->
                            ...
                            <div class="card strpied-tabled-with-hover" style="padding: 10px 20px;">
                                <div class="card-header ">
                                    <div class="float-left">
                                        <h4 class="card-title">Progress Module <?=!empty($_GET['pilihan'])?"Pilihan":"Wajib"?></h4>
                                        <!-- <p class="card-category">List of Module Data</p> -->
                                    </div>
                                    <div class="float-right">
                                        <div class="btn-group" role="group">
                                            <button id="btnGroupDrop1" type="button" class="btn btn-secondary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                Pilih Kelas (Wajib / Pilihan)
                                            </button>
                                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                <a class="dropdown-item" href="admin_progress.php?prg=prg">Kelas Wajib</a>
                                                <a class="dropdown-item" href="admin_progress.php?prg=prg&pilihan=1">Kelas Pilihan</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-hover table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <?php if(empty($_GET['pilihan'])): ?><th class="text-center">Golongan</th><?php endif; ?>
                                        ...
                                        <tbody>
                                            <?php $i=$pagination['start']+1; while($data = sqlsrv_fetch_array($data_module, SQLSRV_FETCH_ASSOC)): ?>
                                                <tr style="cursor:pointer;">
                                                    ...
                                                    <?php if(empty($_GET['pilihan'])): ?><td width="20px" class="text-center"><?=$data['golongan']?></td><?php endif; ?>
                                                    <td><?=$data['title']?></td>
                                                    ...
                                                    <td class="text-right">
                                                        <a class="btn btn-info" style="color:white; width:100px;" href="admin_progress_det.php?prg=prg&id=<?=$data['id']?>">
                                        ...
                                        <ul class="pagination float-right">
                                            <li class="paginate_button page-item <?=$pagination['page']==1?'disabled':''?>">
                                                <a href="admin_progress.php?page=<?=$pagination['page']!=1?$pagination['page']-1:''?>" class="page-link">Previous</a>
                                            </li>
                                            <?php for($i=$pagination['first'];$i<=$pagination['last'];$i++): ?>
                                                <li class="paginate_button page-item <?=$pagination['page']==$i?'active':''?>">
                                                    <a href="admin_progress.php?page=<?=$i?>" class="page-link"><?=$i?></a>
                                                </li>
                                            <?php endfor; ?>
                                            <li class="paginate_button page-item <?=$pagination['page']>=$pagination['total_page']?'disabled':''?>">
                                                <a href="admin_progress.php?page=<?=$pagination['page']<$pagination['total_page']?$pagination['page']+1:''?>" class="page-link">Next</a>
                                            </li>
                                        </ul> 
