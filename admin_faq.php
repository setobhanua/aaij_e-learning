<?php
    ...
    if(!empty($_GET['id']) && !empty($_GET['do']) && $_GET['do'] = "delete"){
        ...
        echo "<script>
                alert('".$message."');
                window.location.href='admin_faq.php';
            </script>";
    }

	$sql = "SELECT * FROM el_faq ORDER BY tanggal DESC";
...
<body>
    <div class="wrapper">
        ...
        <?php
        $page = 'admin_faq'; // current page is about, do the same for other page
        $collapsePage = 'menu3';
        include('sidebar.php');
        ?>
        <!-- Include End Sidebar -->
        ...
        <div class="main-panel">
			...
			<div class="card">
				<div class="card-header ">
					<h4 class="card-title">FAQ Admin</h4>
				</div>
				<div class="row">
					<div class="col-md-12">
							...
							<div class="card-body table-striped table-no-bordered table-hover dataTable">
								...
								<div class="fresh-datatables">
									<table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
										...
										<tbody>
											<?php $i=1; while ($fetch = sqlsrv_fetch_array($query)) :?>
												
											<tr>
                                                <td class="text-center"><?=$i++?></td>
												<td><?= $fetch['nama'] ?></td>
												<td><?= $fetch['email'] ?></td>
												<td width="50%"><?= $fetch['pertanyaan'] ?></td>
                                                <td><?=!empty($fetch['tanggal'])?$fetch['tanggal']->format("d M Y H:i"):'-'?></td>
												<td>
													<a class="btn btn-danger" style="color:white; width:50px;" href="admin_faq.php?do=delete&id=<?=$fetch['id']?>">
