<?php
    ...
    if(!empty($_GET['id']) && !empty($_GET['do']) && $_GET['do'] = "delete"){
        ...
        if(!empty($data)){
            ...
            $sql = "DELETE FROM el_ebook WHERE id=".$_GET['id'];
...
<body>
    <div class="wrapper">
        ...
        <?php
        $page = 'admin_ebook'; // current page is about, do the same for other page
        $collapsePage = 'menu3';
        include('sidebar.php');
        ?>
        <!-- Include End Sidebar -->
                            ...
                            <div class="card strpied-tabled-with-hover" style="padding: 10px 20px;">
                                <div class="card-header ">
                                    ...
                                    <div class="float-right">
                                        <a href="admin_ebook_create.php" class="btn btn-primary">+ Tambah</a>
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-hover table-striped">
                                        ...
                                        <tbody>
                                            <?php $i=$pagination['start']+1; while($data = sqlsrv_fetch_array($data_book, SQLSRV_FETCH_ASSOC)): ?>
                                                <tr style="cursor:pointer;">
                                                    ...
                                                    <td class="text-right">
                                                        <a class="btn btn-info" style="color:white; width:100px;" href="admin_ebook_edit.php?id=<?=$data['id']?>">
                                                            <i class="fa fa-pencil"></i> Edit
                                                        </a>
                                                        <a class="btn btn-danger" style="color:white; width:100px;" href="admin_ebook.php?do=delete&id=<?=$data['id']?>">
                                        ...
                                        <ul class="pagination float-right">
                                            <li class="paginate_button page-item <?=$pagination['page']==1?'disabled':''?>">
                                                <a href="admin_ebook.php?page=<?=$pagination['page']!=1?$pagination['page']-1:''?>" class="page-link">Previous</a>
                                            </li>
                                            <?php for($i=$pagination['first'];$i<=$pagination['last'];$i++): ?>
                                                <li class="paginate_button page-item <?=$pagination['page']==$i?'active':''?>">
                                                    <a href="admin_ebook.php?page=<?=$i?>" class="page-link"><?=$i?></a>
                                                </li>
                                            <?php endfor; ?>
                                            <li class="paginate_button page-item <?=$pagination['page']>=$pagination['total_page']?'disabled':''?>">
                                                <a href="admin_ebook.php?page=<?=$pagination['page']<$pagination['total_page']?$pagination['page']+1:''?>" class="page-link">Next</a>
                                            </li>
                                        </ul>
                                </div>
                            </div>
