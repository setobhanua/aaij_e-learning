<?php
    ...
    $data_pengumuman = sqlsrv_query($conn, $sql, array(),['Scrollable' => SQLSRV_CURSOR_KEYSET]);
    // $data_pengumuman = sqlsrv_num_rows($data_pengumuman);
    // $data_pengumuman = sqlsrv_fetch_array($data_pengumuman, SQLSRV_FETCH_ASSOC);

    // print_debug($sql, false);
    // print_debug($data_pengumuman);
									...
									<table class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                                        ...
                                        <ul class="pagination float-right">
                                            <li class="paginate_button page-item <?=$pagination['page']==1?'disabled':''?>">
                                                <a href="pengumuman.php?page=<?=$pagination['page']!=1?$pagination['page']-1:''?>" class="page-link">Previous</a>
                                            </li>
                                            <?php for($i=$pagination['first'];$i<=$pagination['last'];$i++): ?>
                                                <li class="paginate_button page-item <?=$pagination['page']==$i?'active':''?>">
                                                    <a href="pengumuman.php?page=<?=$i?>" class="page-link"><?=$i?></a>
                                                </li>
                                            <?php endfor; ?>
                                            <li class="paginate_button page-item <?=$pagination['page']>=$pagination['total_page']?'disabled':''?>">
                                                <a href="pengumuman.php?page=<?=$pagination['page']<$pagination['total_page']?$pagination['page']+1:''?>" class="page-link">Next</a>
                                            </li>
                                        </ul>
								</div>
							</div>
